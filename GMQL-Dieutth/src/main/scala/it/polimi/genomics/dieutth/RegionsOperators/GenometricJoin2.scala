package it.polimi.genomics.dieutth.RegionsOperators

import com.google.common.base.Charsets
import com.google.common.hash.Hashing
import it.polimi.genomics.core.DataStructures.JoinParametersRD.RegionBuilder.RegionBuilder
import it.polimi.genomics.core.DataStructures.JoinParametersRD._
import it.polimi.genomics.core.DataStructures.{MetaJoinOperator, OptionalMetaJoinOperator, RegionOperator, SomeMetaJoinOperator}
 ...
 **/
object GenometricJoin2 {
  private final val logger = LoggerFactory.getLogger(this.getClass);

  @throws[SelectFormatException]
  def apply(executor : GMQLSparkDieuExecutor, metajoinCondition : OptionalMetaJoinOperator, joinCondition : List[JoinQuadruple], regionBuilder : RegionBuilder, leftDataset : RegionOperator, rightDataset : RegionOperator, BINNING_PARAMETER:Long, MAXIMUM_DISTANCE:Long, sc : SparkContext) : RDD[GRECORD] = {
    // load datasets
    val ref : RDD[GRECORD] =
...
//      joined.collect().foreach(x=>println("join: "+x))
      val firstRound: RDD[(Long, (Long, String, Long, Long, Char, Array[GValue], Long, Long, Char, Array[GValue], Long))] =
        joined .flatMap{x=> val r = x._2._1; val e = x._2._2;
          ...
          val intersect_distance = (!firstRoundParameters.max.isEmpty || firstRoundParameters.max.get > distance) && (!firstRoundParameters.min.isEmpty || firstRoundParameters.min.get < distance)
      ...
      val minDistance: RDD[(Long, (Long, String, Long, Long, Char, Array[GValue], Long, Long, Char, Array[GValue], Long))] = //: DataSet[(Long, Long, String, Long, Long, Char, Array[GValue], Long, Long, Char, Array[GValue], Long)] =
        if (minDistanceParameter.isDefined) {
          firstRound.groupByKey()
            ...
            itr.takeWhile(s=> {if(count >0 && s._11 != buffer) {count = count -1 ;buffer = s._11   ; true} else if (s._11==buffer) true else  false})/*take(minDistanceParameter.get.asInstanceOf[MinDistance].number)*/.map(s=> (x._1,s))}
        } else {
          firstRound
        }
  ...
  def prepareDs(ds : RDD[( Long,Long, String, Long, Long, Char, Array[GValue])],firstRound : JoinExecutionParameter, secondRound : JoinExecutionParameter,binSize : Long, max : Long) : RDD[((Long, String,Int),( Long,Long, Long, Char, Array[GValue], Int))] = {
    ds.flatMap{r  =>
      ...
      val maxDistance : Long =
        if(firstRound.max.isEmpty) firstRound.max.get
        else if(secondRound.max.isEmpty) Math.max(secondRound.max.get,max)
        else max
...
//      if(!stream.isDefined || stream.get.equals('+')){
  ...
  def binExperiment(ds: RDD[GRECORD], Bgroups:  Option[Broadcast[Map[Long, Iterable[Long]]]],BINNING_PARAMETER:Long): RDD[((Long, String, Int), ( Long,Long, Long, Char, Array[GValue], Int,Int))] = {
