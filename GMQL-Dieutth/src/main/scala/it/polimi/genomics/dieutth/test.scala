...
import it.polimi.genomics.dieutth.GMQLSparkDieuExecutor.GMQL_DATASET
import it.polimi.genomics.dieutth.RegionsOperators.MapWithCustomPartitioner.CustomPartitioner
import it.polimi.genomics.dieutth.loaders.CustomParser
import org.apache.spark.{SparkConf, SparkContext}

object test {
  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setAppName("Partitioner, updated Gmap71").setMaster("local[4]")
    ...
    val server = new GmqlServer(new GMQLSparkDieuExecutor(sc=sc,outputFormat = GMQLSchemaFormat.TAB))
